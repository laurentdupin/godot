...
const WIDTH := 640
const HEIGHT := 360
const SVG_DATA := ""
...
func _run() -> void:
	...
	Input.warp_mouse(Vector2(620, 340))
...
.slot{position:absolute;left:170px;top:80px;width:300px;height:200px}
...
.border{position:absolute;left:0;top:0;width:300px;height:200px;object-fit:fill;transition:left 200ms linear,top 200ms linear,width 200ms linear,height 200ms linear}
.slot.hot .content{transform:scale(1.1)}
.slot.hot .border{left:-15px;top:-10px;width:330px;height:220px}
	...
	var baseline_top := _find_top_stroke(baseline, 320, 65, 90)
	var baseline_thickness := _vertical_stroke_thickness(baseline, 320, baseline_top)
	...
	var hovered_top := _find_top_stroke(hovered, 320, 60, 90)
	var hovered_thickness := _vertical_stroke_thickness(hovered, 320, hovered_top)

	if baseline_top < 0 or hovered_top < 0 or hovered_top > baseline_top - 3:
	...
	if baseline_thickness <= 0 or abs(baseline_thickness - hovered_thickness) > 1:
		_fail("%s SVG vector-effect stroke changed thickness: %dpx -> %dpx." % [backend_name, baseline_thickness, hovered_thickness])
		return
	print("HCSR Godot %s non-scaling SVG stroke smoke passed: thickness=%dpx->%dpx max_record=%.3fms." % [backend_name, baseline_thickness, hovered_thickness, maximum_record_seconds * 1000.0])

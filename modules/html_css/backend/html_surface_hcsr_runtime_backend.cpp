...
enum RuntimeMutationKind {
	...
	RUNTIME_MUTATION_VALUE,
	RUNTIME_MUTATION_CHECKED,
	RUNTIME_MUTATION_INNER_HTML,
};

static constexpr int RUNTIME_INTERACTIVE_STEP_SLICE_UNITS = 65536;
static constexpr int RUNTIME_PRESENTER_STEP_SLICE_UNITS = 1;
static constexpr int RUNTIME_INTERACTIVE_PRESENTER_MAX_SLICES = 1;
static constexpr int RUNTIME_PRESENTER_STANDBY_STEP_UNITS = 1;
static constexpr int RUNTIME_SEMANTIC_STEP_SLICE_UNITS = 65536;
static constexpr uint64_t RUNTIME_SEMANTIC_FRAME_BUDGET_MICROSECONDS = 4000;
static constexpr int RUNTIME_PRESENTER_SHUTDOWN_STEP_UNITS = 1;
static constexpr uint64_t RUNTIME_INTERACTIVE_FRAME_BUDGET_MICROSECONDS = 2000;
static constexpr int RUNTIME_ADMITTED_SCROLL_INPUT_CAPACITY = 256;
static constexpr int RUNTIME_PENDING_MUTATION_CAPACITY = 4096;
static constexpr int RUNTIME_POINTER_EVENT_CAPACITY = 256;
static constexpr uint32_t RUNTIME_REQUIRED_ABI_VERSION = 20;

static_assert(HCSR_RUNTIME_ABI_VERSION == RUNTIME_REQUIRED_ABI_VERSION,
		"The Godot HCSR replacement backend must be compiled against runtime ABI v20.");
...
static bool runtime_mutations_are_equivalent(
		const RuntimeMutation &p_left, const RuntimeMutation &p_right) {
	return p_left.kind == p_right.kind
			&& p_left.id == p_right.id
			&& p_left.name == p_right.name
			&& p_left.value == p_right.value
			&& p_left.causal_host_receipt_id == p_right.causal_host_receipt_id
			&& p_left.causal_receipt_timestamp_microseconds == p_right.causal_receipt_timestamp_microseconds;
}

static bool runtime_mutations_share_scalar_slot(
		const RuntimeMutation &p_left, const RuntimeMutation &p_right) {
	return p_left.kind != RUNTIME_MUTATION_INNER_HTML
			&& p_right.kind != RUNTIME_MUTATION_INNER_HTML
			&& p_left.causal_host_receipt_id == 0
			&& p_right.causal_host_receipt_id == 0
			&& p_left.kind == p_right.kind
			&& p_left.id == p_right.id
			&& p_left.name == p_right.name;
}

static bool runtime_coalesce_pending_scalar_mutation(
		Vector<RuntimeMutation> &p_pending,
		const RuntimeMutation &p_mutation) {
	for (int index = p_pending.size() - 1; index >= 0; index--) {
		const RuntimeMutation &pending = p_pending[index];
		if (pending.kind == RUNTIME_MUTATION_INNER_HTML) {
			break;
		}
		if (runtime_mutations_are_equivalent(pending, p_mutation)) {
			return true;
		}
		if (runtime_mutations_share_scalar_slot(pending, p_mutation)) {
			p_pending.remove_at(index);
			p_pending.push_back(p_mutation);
			return true;
		}
	}
	return false;
...
struct RuntimePointerRequest {
	...
	bool source_authority_refreshed = false;
...
static Vector<RuntimePlatformFontSource> runtime_discover_platform_fonts() {
	static const char *families[] = {
		...
		"Consolas", "Courier New", "monospace", "ui-monospace",
	};
...
struct HTMLSurfaceHCSRRuntimeBackend::RuntimeState {
	...
	bool input_wakeup_scheduled = false;
	...
	uint64_t submitted_animation_timeline_revision = 0;
	uint64_t accepted_animation_timeline_revision = 0;
	int64_t requested_animation_time_microseconds = 0;
	int64_t animation_clock_origin_microseconds = -1;
	uint64_t animation_timeline_document_request_id = 0;
	...
	hcsr_runtime_scroll_orientation_t scrollbar_interaction_orientation = HCSR_RUNTIME_SCROLL_VERTICAL;
...
static String runtime_copy_presenter_error(
		hcsr_runtime_d3d12_presenter_t *p_presenter,
		const String &p_fallback) {
	Vector<char> error_buffer;
	error_buffer.resize(16384);
	if (hcsr_runtime_d3d12_presenter_copy_last_error(
			p_presenter, error_buffer.ptrw(), error_buffer.size()) != HCSR_RUNTIME_OK) {
		return p_fallback;
	}
	const String error = String::utf8(error_buffer.ptr());
	return error.is_empty() ? p_fallback : error;
}
...
static bool runtime_ensure_initialized(
	...
	if (hcsr_runtime_get_abi_version() != RUNTIME_REQUIRED_ABI_VERSION) {
		runtime_set_terminal(p_state, "HCSR replacement ABI mismatch; Godot requires runtime ABI v20.");
		return false;
	}
...
static bool runtime_submit_mutations(
	...
	for (const RuntimeMutation &mutation : p_mutations) {
		if (r_consumed_count >= HCSR_RUNTIME_MAX_MUTATION_OPERATIONS) {
			break;
		}
		...
		switch (mutation.kind) {
			...
			case RUNTIME_MUTATION_STYLE:
				...
				break;
			case RUNTIME_MUTATION_VALUE:
				status = hcsr_runtime_mutation_set_control_value_by_id(journal, id.get_data(), value.get_data());
				break;
		...
		if (status == HCSR_RUNTIME_LIMIT_EXCEEDED && r_consumed_count > 0) {
			break;
		}
		if (status == HCSR_RUNTIME_COMPILE_FAILED
				&& mutation.causal_host_receipt_id == 0) {
			// A queued local mutation can become stale after an earlier structural
			// journal removes its target. It was valid when Godot accepted it, so
			// retire it as an obsolete no-op instead of terminalizing the session.
			r_consumed_count++;
			continue;
		}
		if (status != HCSR_RUNTIME_OK) {
	...
	(void)p_interactive;
	const bool interactive = !causal_receipts.is_empty();
...
static bool runtime_service_presenter_standby(RuntimePresentationBinding *p_binding) {
	...
	hcsr_runtime_step_info_t step;
	initialize_abi(&step, sizeof(step));
	const hcsr_runtime_status_t standby_status = hcsr_runtime_d3d12_presenter_step(
			p_binding->presenter, RUNTIME_PRESENTER_STANDBY_STEP_UNITS, &step);
	if (standby_status != HCSR_RUNTIME_OK && standby_status != HCSR_RUNTIME_PENDING) {
		return false;
	}
	initialize_abi(&diagnostics, sizeof(diagnostics));
	if (hcsr_runtime_d3d12_presenter_query_diagnostics(p_binding->presenter, &diagnostics) != HCSR_RUNTIME_OK) {
...
static bool runtime_step_retiring_binding(
	...
	if (p_binding->presenter != nullptr) {
		...
		const hcsr_runtime_status_t status = hcsr_runtime_d3d12_presenter_step_shutdown(
				p_binding->presenter, RUNTIME_PRESENTER_SHUTDOWN_STEP_UNITS, &step);
...
static bool runtime_step_active(
	...
	bool semantic_pending = p_state->semantic_pending;
	hcsr_runtime_status_t status = HCSR_RUNTIME_OK;
	if (!publication_presentation_pending && p_state->interactive_pending) {
		hcsr_runtime_interactive_step_info_t interactive;
		initialize_abi(&interactive, sizeof(interactive));
		status = hcsr_runtime_session_step_interactive_with_budget(
				p_state->session, RUNTIME_INTERACTIVE_STEP_SLICE_UNITS,
				RUNTIME_INTERACTIVE_FRAME_BUDGET_MICROSECONDS, &interactive);
	...
	} else if (!publication_presentation_pending && p_state->semantic_pending) {
	...
	if (p_state->active_binding->presenter_pending) {
		...
		if (status != HCSR_RUNTIME_OK && status != HCSR_RUNTIME_PENDING) {
			runtime_set_terminal(p_state, runtime_copy_presenter_error(
					p_state->active_binding->presenter,
					vformat("HCSR replacement D3D12 presenter failed with status %d.", (int)status)));
			return false;
		}
		if (status == HCSR_RUNTIME_OK) {
	...
	if (p_state->successor_binding != nullptr
		...
		if (status != HCSR_RUNTIME_OK && status != HCSR_RUNTIME_PENDING) {
			runtime_set_terminal(p_state, runtime_copy_presenter_error(
					p_state->successor_binding->presenter,
					vformat("HCSR replacement successor D3D12 presenter failed with status %d.", (int)status)));
			return false;
		}
		if (status == HCSR_RUNTIME_OK) {
	...
	for (const RuntimeOutputSnapshotEntry &entry : p_state->staged_topology.outputs) {
		...
		if (status != HCSR_RUNTIME_OK && status != HCSR_RUNTIME_PENDING) {
			runtime_set_terminal(p_state, runtime_copy_presenter_error(
					binding->presenter,
					vformat("HCSR replacement secondary D3D12 presenter failed with status %d.", (int)status)));
			return false;
		}
		if (status == HCSR_RUNTIME_OK) {
	...
	hcsr_runtime_animation_schedule_info_t animation_schedule;
	initialize_abi(&animation_schedule, sizeof(animation_schedule));
	if (hcsr_runtime_session_get_animation_schedule(
				p_state->session, &animation_schedule) != HCSR_RUNTIME_OK) {
		runtime_set_terminal(p_state, "Godot could not query the HCSR animation schedule.");
		return false;
	}
	{
		MutexLock lock(p_state->mutex);
		p_state->animation_needs_next_frame = animation_schedule.needs_next_frame != 0;
		if (animation_schedule.document_request_id == p_state->animation_timeline_document_request_id) {
			p_state->accepted_animation_timeline_revision = MAX(
					p_state->accepted_animation_timeline_revision,
					animation_schedule.revision_id);
		}
...
static bool runtime_create_pointer_event_target(
	...
	initialize_abi(&target, sizeof(target));
	if (p_state->active_publication == nullptr || p_state->active_generation == 0
			|| hcsr_runtime_publication_hit_test(p_state->active_publication, p_state->active_generation,
					p_position.x, p_position.y, &target) != HCSR_RUNTIME_OK) {
		return false;
	}
	char element_id[4096] = {};
	if (hcsr_runtime_publication_copy_element_id(p_state->active_publication, p_state->active_generation,
			target.owner_node_epoch, target.owner_node_id, element_id, sizeof(element_id)) != HCSR_RUNTIME_OK
			|| element_id[0] == '\0') {
		return false;
	}
	r_hit = HTMLElementHit();
	r_hit.element_id = StringName(String::utf8(element_id));
	const int left = Math::floor(target.logical_x);
	const int top = Math::floor(target.logical_y);
	const int right = Math::ceil(target.logical_x + target.logical_width);
	const int bottom = Math::ceil(target.logical_y + target.logical_height);
	r_hit.bounds = Rect2i(left, top, MAX(0, right - left), MAX(0, bottom - top));
	return true;
}
...
static bool runtime_step_pointer_input(HTMLSurfaceHCSRRuntimeBackend::RuntimeState *p_state) {
	if (p_state->active_pointer_submission_id == 0) {
		...
		input.input_id = MAX(p_state->next_host_input_id, request.source_input_id + 1);
		p_state->next_host_input_id = input.input_id + 1;
		input.frame_id = MAX(p_state->next_host_frame_id, request.source_frame_id + 1);
		p_state->next_host_frame_id = input.frame_id + 1;
		...
		if ((submit_status == HCSR_RUNTIME_STALE_REQUEST
				|| submit_status == HCSR_RUNTIME_GENERATION_MISMATCH)
				&& request.kind == HCSR_RUNTIME_POINTER_MOVE) {
			hcsr_runtime_host_input_binding_t binding;
			initialize_abi(&binding, sizeof(binding));
			binding.receipt_id = request.host_receipt_id;
		...
		if (submit_status == HCSR_RUNTIME_PENDING
				|| submit_status == HCSR_RUNTIME_NO_NEW_PUBLICATION
				|| submit_status == HCSR_RUNTIME_LIMIT_EXCEEDED
				|| submit_status == HCSR_RUNTIME_STALE_REQUEST
				|| submit_status == HCSR_RUNTIME_GENERATION_MISMATCH) {
			MutexLock lock(p_state->mutex);
	...
	if (step_status == HCSR_RUNTIME_NO_NEW_PUBLICATION
			|| step_status == HCSR_RUNTIME_STALE_REQUEST
			|| step_status == HCSR_RUNTIME_GENERATION_MISMATCH) {
		p_state->active_pointer_submission_id = 0;
		bool resolved_stale_move = false;
		if (p_state->active_pointer_request_valid
				&& p_state->active_pointer_request.kind == HCSR_RUNTIME_POINTER_MOVE) {
			hcsr_runtime_host_input_binding_t binding;
			initialize_abi(&binding, sizeof(binding));
			binding.receipt_id = p_state->active_pointer_request.host_receipt_id;
			binding.publication_lane = HCSR_RUNTIME_HOST_INPUT_POINTER;
			binding.resolved_without_mutation = 1;
			resolved_stale_move = hcsr_runtime_session_resolve_host_input(p_state->session, &binding) == HCSR_RUNTIME_OK;
		}
		if (p_state->active_pointer_request_valid && !resolved_stale_move) {
			MutexLock lock(p_state->mutex);
	...
	const RuntimePointerRequest completed_request = p_state->active_pointer_request;
	if (completed_request.kind == HCSR_RUNTIME_POINTER_PRIMARY_DOWN
			|| completed_request.kind == HCSR_RUNTIME_POINTER_PRIMARY_UP) {
		HTMLPointerEvent event;
		event.sequence = ++p_state->pointer_event_sequence;
		event.type = completed_request.kind == HCSR_RUNTIME_POINTER_PRIMARY_DOWN
				? HTML_POINTER_EVENT_DOWN : HTML_POINTER_EVENT_UP;
		event.document_position = completed_request.position;
		event.button = HTML_SURFACE_MOUSE_BUTTON_LEFT;
		event.buttons = completed_request.buttons;
		event.state_changed = true;
		runtime_create_pointer_event_target(p_state, completed_request.position, event.target);
		MutexLock lock(p_state->mutex);
		if (p_state->pointer_events.size() < RUNTIME_POINTER_EVENT_CAPACITY) {
			p_state->pointer_events.push_back(event);
		}
		if (completed_request.kind == HCSR_RUNTIME_POINTER_PRIMARY_UP
				&& step.activation_action_id != 0
				&& p_state->pointer_events.size() < RUNTIME_POINTER_EVENT_CAPACITY) {
			event.sequence = ++p_state->pointer_event_sequence;
			event.type = HTML_POINTER_EVENT_CLICK;
			p_state->pointer_events.push_back(event);
		}
	}
	p_state->active_pointer_submission_id = 0;
...
static bool runtime_submit_one_scroll_input(HTMLSurfaceHCSRRuntimeBackend::RuntimeState *p_state) {
	...
	const hcsr_runtime_status_t status = hcsr_runtime_session_submit_scroll_input(
			p_state->session, &input, &submission);
	if (status == HCSR_RUNTIME_PENDING
			|| status == HCSR_RUNTIME_NO_NEW_PUBLICATION
			|| status == HCSR_RUNTIME_LIMIT_EXCEEDED
			|| status == HCSR_RUNTIME_STALE_REQUEST
			|| status == HCSR_RUNTIME_GENERATION_MISMATCH) {
		MutexLock lock(p_state->mutex);
...
static bool runtime_seal_host_frame_requirements(
	...
	seal.timeline_revision = p_state->staged_lineage.animation_timeline_revision;
	...
	seal.publication_pointer_input_revision = p_state->staged_lineage.interaction_input_id;
	seal.publication_scroll_input_revision = p_state->staged_lineage.scroll_input_id;
	seal.publication_author_revision = p_state->staged_lineage.target_author_revision;
	seal.seal_through_publication_authority = 1;
...
static bool runtime_submit_pending_animation_timeline(
	...
	{
		MutexLock lock(p_state->mutex);
		if (!p_state->animation_timeline_dirty || p_state->configuration_dirty || p_state->interactive_pending || p_state->activation_pending || p_state->successor_binding != nullptr) {
			return true;
		}
	...
	if (p_state->requested_animation_timeline_revision == revision) {
		p_state->submitted_animation_timeline_revision = revision;
...
void HTMLSurfaceHCSRRuntimeBackend::_step_on_render_thread_callback(uint64_t p_state_ptr) {
	RuntimeState *runtime = (RuntimeState *)p_state_ptr;
	{
		...
		runtime->input_wakeup_scheduled = false;
	}
	Vector<RuntimeMutation> mutations;
	uint64_t mutation_request_process_frame = 0;
	uint64_t document_request_process_frame = 0;
	...
	auto finish_scheduled_work = [runtime]() {
		...
		{
			MutexLock lock(runtime->mutex);
			// A live HTMLView/HTMLRenderTarget polls pending work from its engine
			// frame. Re-enqueuing every semantic slice here can monopolize the
			// render thread for an entire mutation backlog and stall unrelated
			// Godot presentation. Closing has no owning node left to drive bounded
			// retirement, so teardown retains its independent continuation.
			defer_next_slice = runtime->closing && runtime->work_reschedule_requested;
	...
	for (int input_sweep = 0; input_sweep < 64; input_sweep++) {
		int pointer_count_before = 0;
		int scroll_count_before = 0;
		{
			MutexLock lock(runtime->mutex);
			pointer_count_before = runtime->pointer_requests.size();
			scroll_count_before = runtime->scroll_requests.size();
		}
		if (!runtime_step_pointer_input(runtime) || !runtime_submit_one_scroll_input(runtime)) {
			finish_scheduled_work();
			return;
		}
		bool derivation_started = false;
		bool input_was_consumed = false;
		{
			MutexLock lock(runtime->mutex);
			derivation_started = runtime->active_pointer_submission_id != 0
					|| runtime->interactive_pending || runtime->semantic_pending;
			input_was_consumed = runtime->pointer_requests.size() < pointer_count_before
					|| runtime->scroll_requests.size() < scroll_count_before;
		}
		if (derivation_started || !input_was_consumed) {
			break;
		}
	}
...
void HTMLSurfaceHCSRRuntimeBackend::_activate_frame_cutoff_on_render_thread_callback(uint64_t p_state_ptr) {
	...
	if (staged_configuration_superseded) {
		...
		runtime_schedule_state(runtime);
		return;
	}
	...
	const bool exact_interactive_frame = runtime->activation_pending
			...
			&& runtime->staged_lineage.interactive_submission_id == runtime->newest_requested_submission_id;
	// Render-thread callbacks routinely cross a Godot process-frame boundary.
	// Exact publication authority and the native host-frame permit remain the
	// correctness gate; the process-frame identity only bounds activation rate.
	const bool activation_turn_is_stale = runtime->activation_pending
			...
			&& current_process_frame == runtime->last_activation_process_frame;
	...
	if (runtime->activation_pending
		...
		hcsr_runtime_animation_schedule_info_t activated_animation_schedule;
		initialize_abi(&activated_animation_schedule, sizeof(activated_animation_schedule));
		if (hcsr_runtime_session_get_animation_schedule(
				runtime->session, &activated_animation_schedule) != HCSR_RUNTIME_OK) {
			runtime_set_terminal(runtime, "Godot could not refresh the activated HCSR animation schedule.");
			return;
		}
		{
			MutexLock lock(runtime->mutex);
			...
			if (runtime->animation_timeline_document_request_id
					!= runtime->staged_lineage.document_request_id) {
				runtime->animation_timeline_document_request_id = runtime->staged_lineage.document_request_id;
				runtime->requested_animation_timeline_revision++;
				runtime->submitted_animation_timeline_revision = runtime->accepted_animation_timeline_revision;
				runtime->requested_animation_time_microseconds = 0;
				runtime->animation_clock_origin_microseconds = -1;
				runtime->animation_timeline_dirty = true;
				runtime->pending_work = true;
			}
			...
			runtime->accepted_animation_timeline_revision = MAX(
					runtime->accepted_animation_timeline_revision,
					runtime->staged_lineage.animation_timeline_revision);
			if (activated_animation_schedule.document_request_id
					== runtime->staged_lineage.document_request_id) {
				runtime->accepted_animation_timeline_revision = MAX(
						runtime->accepted_animation_timeline_revision,
						activated_animation_schedule.revision_id);
				runtime->animation_needs_next_frame = activated_animation_schedule.needs_next_frame != 0;
			}
		...
		runtime->last_activation_process_frame = current_process_frame;
	}
	bool resume_pending_work = false;
	{
		MutexLock lock(runtime->mutex);
		runtime->pending_work = (runtime->active_binding != nullptr && runtime->active_binding->presenter_pending)
				...
				|| runtime->retiring_binding != nullptr
				|| runtime->semantic_pending
				|| runtime->animation_timeline_dirty
				|| runtime->active_pointer_submission_id != 0
				|| !runtime->mutations.is_empty()
				|| !runtime->pointer_requests.is_empty()
				|| !runtime->scroll_requests.is_empty()
		...
		resume_pending_work = runtime->pending_work;
	}
	if (resume_pending_work) {
		runtime_schedule_state(runtime);
	}
}
...
Error HTMLSurfaceHCSRRuntimeBackend::_queue_mutation(
	...
	mutation.value = p_value;
	{
		...
		if (runtime_coalesce_pending_scalar_mutation(state->mutations, mutation)) {
			return OK;
		}
		if (state->mutations.size() >= RUNTIME_PENDING_MUTATION_CAPACITY) {
			return ERR_BUSY;
		}
...
void HTMLSurfaceHCSRRuntimeBackend::set_viewport_configuration(
	...
	bool changed = false;
	{
		MutexLock lock(state->mutex);
		const Size2i requested_size = Size2i(MAX(1, p_size.x), MAX(1, p_size.y));
		const Size2i requested_physical_size = Size2i(
				MAX(1, p_physical_size.x), MAX(1, p_physical_size.y));
		if (state->logical_size != requested_size
				|| state->physical_size != requested_physical_size) {
			state->logical_size = requested_size;
			state->physical_size = requested_physical_size;
			state->configuration_dirty = true;
			state->configuration_request_process_frame = Engine::get_singleton()->get_process_frames() + 1;
...
Error HTMLSurfaceHCSRRuntimeBackend::update_compositor(
	...
	if (state != nullptr) {
		MutexLock lock(state->mutex);
		if (state->animation_needs_next_frame && state->active_generation != 0 && !state->configuration_dirty && !state->interactive_pending && !state->activation_pending && !state->animation_timeline_dirty && state->successor_binding == nullptr && state->submitted_animation_timeline_revision <= state->accepted_animation_timeline_revision) {
			const int64_t host_time_microseconds = MAX((int64_t)0,
					(int64_t)(MAX(0.0, p_timeline_time_seconds) * 1000000.0));
			if (state->animation_clock_origin_microseconds < 0) {
				state->animation_clock_origin_microseconds = host_time_microseconds;
			}
			const int64_t time_microseconds = MAX((int64_t)0,
					host_time_microseconds - state->animation_clock_origin_microseconds);
...
bool HTMLSurfaceHCSRRuntimeBackend::has_pending_output() const {
	...
	MutexLock lock(state->mutex);
	return state->presentation_changed || state->pending_work || state->work_scheduled || state->document_dirty
			|| state->activation_pending || state->activation_deferred_to_next_process_frame
...
static Error runtime_queue_pointer_request(
	...
	bool schedule_input_wakeup = false;
	{
		MutexLock lock(p_state->mutex);
		...
		if (receipt_status != HCSR_RUNTIME_OK) {
			if (receipt_status == HCSR_RUNTIME_LIMIT_EXCEEDED) {
				return ERR_BUSY;
			}
			p_state->terminal = true;
			p_state->pending_work = false;
			p_state->terminal_reason = runtime_copy_last_error(p_state, vformat(
					"HCSR rejected pointer input at host receipt (status %d).",
					(int)receipt_status));
		...
		request.source_frame_id = p_state->active_interaction_frame_id;
		...
		if (!p_state->active_has_interaction_state) {
			...
			if (p_kind == HCSR_RUNTIME_POINTER_MOVE && !p_state->pointer_requests.is_empty()
				...
				const RuntimePointerRequest &predecessor =
						p_state->pointer_requests[p_state->pointer_requests.size() - 1];
				const bool same_authority = !predecessor.source_authority_refreshed
						&& !request.source_authority_refreshed
						&& predecessor.source_runtime_generation == request.source_runtime_generation
						&& predecessor.source_configuration_id == request.source_configuration_id
						&& predecessor.source_input_id == request.source_input_id;
				if (!same_authority) {
					hcsr_runtime_host_input_binding_t binding;
					initialize_abi(&binding, sizeof(binding));
					binding.receipt_id = predecessor_receipt_id;
					binding.publication_lane = HCSR_RUNTIME_HOST_INPUT_POINTER;
					binding.resolved_without_mutation = 1;
					if (hcsr_runtime_session_resolve_host_input(p_state->session, &binding) == HCSR_RUNTIME_OK) {
						request.receipt_timestamp_microseconds = MIN(
								request.receipt_timestamp_microseconds,
								predecessor.receipt_timestamp_microseconds);
						request.first_receipt_sequence = predecessor.first_receipt_sequence;
					...
					if (supersede_status == HCSR_RUNTIME_GENERATION_MISMATCH) {
						p_state->pointer_requests.push_back(request);
					} else {
		...
		p_state->pending_work = true;
		if (p_state->work_scheduled && !p_state->input_wakeup_scheduled) {
			p_state->input_wakeup_scheduled = true;
			schedule_input_wakeup = true;
		}
	}
...
static Error runtime_queue_scroll_request(
	...
	{
		MutexLock lock(p_state->mutex);
		if (p_state->closing || p_state->terminal || p_state->session == nullptr || p_state->active_generation == 0 || p_state->frame_stream_epoch == 0) {
			return ERR_UNAVAILABLE;
		}
		const uint64_t host_process_frame = Engine::get_singleton()->get_process_frames() + 1;
		if (p_kind == HCSR_RUNTIME_SCROLL_INPUT_WHEEL && !p_state->scroll_requests.is_empty()) {
			RuntimeScrollRequest &pending = p_state->scroll_requests.write[p_state->scroll_requests.size() - 1];
			if (pending.kind == p_kind && pending.source == p_source && pending.orientation == p_orientation && pending.host_process_frame == host_process_frame && pending.source_runtime_generation == p_state->active_generation && pending.source_configuration_id == p_state->active_scroll_configuration_id && pending.source_input_id == p_state->active_scroll_input_id && pending.receipt_sequence == p_state->next_host_receipt_sequence) {
				pending.position = p_position;
				pending.delta += p_delta;
				p_state->pending_work = true;
				coalesced = true;
			}
		}
		if (coalesced) {
			// Scheduling must happen after releasing the state mutex. The worker can
			// finish between adjacent engine-frame inputs, so the first input's
			// wake-up is not sufficient for the coalesced request.
		} else {
			hcsr_runtime_host_input_receipt_request_t receipt_request;
			initialize_abi(&receipt_request, sizeof(receipt_request));
			receipt_request.frame_stream_epoch = p_state->frame_stream_epoch;
			...
			if (receipt_status != HCSR_RUNTIME_OK) {
				...
				p_state->pending_work = false;
				p_state->terminal_reason = vformat(
						"HCSR rejected scroll input at host receipt (status %d).",
						(int)receipt_status);
				ERR_PRINT(p_state->terminal_reason);
				return ERR_INVALID_DATA;
			}
			...
			request.host_process_frame = host_process_frame;
...
Error HTMLSurfaceHCSRRuntimeBackend::begin_scrollbar_interaction(const Point2 &p_position, double p_event_time_seconds, bool &r_consumed) {
	...
	hcsr_runtime_scroll_orientation_t orientation = HCSR_RUNTIME_SCROLL_VERTICAL;
	{
		MutexLock lock(state->mutex);
		if (state->active_publication == nullptr || state->active_generation == 0) {
			return OK;
		}
		bool hit_scrollbar = false;
		for (int scrollbar_index = 0; scrollbar_index < 256; scrollbar_index++) {
			hcsr_runtime_scrollbar_geometry_info_t geometry;
			initialize_abi(&geometry, sizeof(geometry));
			const hcsr_runtime_status_t status = hcsr_runtime_publication_get_scrollbar_geometry(
					state->active_publication, state->active_generation, scrollbar_index, &geometry);
			...
			const Rect2 track(
					Point2(geometry.track_x, geometry.track_y),
					Size2(geometry.track_width, geometry.track_height));
			if (track.has_point(p_position)) {
				hit_scrollbar = true;
				orientation = (hcsr_runtime_scroll_orientation_t)geometry.orientation;
				break;
			}
		}
		if (!hit_scrollbar) {
		...
		state->scrollbar_interaction_orientation = orientation;
	}
	const Error result = runtime_queue_scroll_request(state, HCSR_RUNTIME_SCROLL_INPUT_SCROLLBAR_BEGIN,
			p_position, Vector2(), HCSR_RUNTIME_SCROLL_SOURCE_SCROLLBAR, orientation);
...
Error HTMLSurfaceHCSRRuntimeBackend::update_scrollbar_interaction(const Point2 &p_position, bool &r_consumed) {
	...
	hcsr_runtime_scroll_orientation_t orientation;
	{
		MutexLock lock(state->mutex);
		if (!state->scrollbar_interaction_active) {
			return OK;
		}
		orientation = state->scrollbar_interaction_orientation;
	}
...
Error HTMLSurfaceHCSRRuntimeBackend::end_scrollbar_interaction(bool &r_consumed) {
	...
	const Error result = runtime_queue_scroll_request(state, HCSR_RUNTIME_SCROLL_INPUT_SCROLLBAR_END,
			position, Vector2(), HCSR_RUNTIME_SCROLL_SOURCE_SCROLLBAR, orientation);
	r_consumed = result == OK;
	return result;
}
...
Error HTMLSurfaceHCSRRuntimeBackend::apply_element_mutations(const Array &p_mutations) {
	...
	}
	{
		MutexLock lock(state->mutex);
		ERR_FAIL_COND_V(state->terminal || state->closing, ERR_UNAVAILABLE);
		if (parsed.size() > RUNTIME_PENDING_MUTATION_CAPACITY - state->mutations.size()) {
			return ERR_BUSY;
		}
		if (state->causal_input_transaction_depth > 0) {
...
bool HTMLSurfaceHCSRRuntimeBackend::get_form_control_state(
	...
	MutexLock lock(state->mutex);
	if (state->closing || state->terminal || state->active_publication == nullptr
			|| state->active_generation == 0) {
		return false;
	}
	CharString id = String(p_id).utf8();
	Vector<char> tag_name;
	tag_name.resize(HCSR_RUNTIME_MAX_FORM_CONTROL_TAG_UTF8_BYTES);
	Vector<char> value;
	value.resize(HCSR_RUNTIME_MAX_FORM_CONTROL_VALUE_UTF8_BYTES);
	hcsr_runtime_form_control_state_info_t source;
	initialize_abi(&source, sizeof(source));
	if (hcsr_runtime_publication_get_form_control_state(
			state->active_publication, state->active_generation, id.get_data(), &source,
			tag_name.ptrw(), tag_name.size(), value.ptrw(), value.size()) != HCSR_RUNTIME_OK) {
		return false;
	}
	r_state = HTMLFormControlState();
	r_state.element_id = p_id;
	r_state.tag_name = StringName(String::utf8(tag_name.ptr()));
	...
	r_state.selection_offsets_present = source.selection_offsets_present != 0;
	r_state.selection_start = source.selection_start;
	r_state.selection_end = source.selection_end;
	return true;
}
...
bool HTMLSurfaceHCSRRuntimeBackend::hit_test(const Point2 &p_position, HTMLElementHit &r_hit) const {
	...
	hcsr_runtime_interaction_target_info_t target;
	initialize_abi(&target, sizeof(target));
	const hcsr_runtime_status_t hit_status = hcsr_runtime_publication_hit_test(
			state->active_publication, state->active_generation, p_position.x, p_position.y, &target);
	if (hit_status != HCSR_RUNTIME_OK) {
		return false;
	}
	char element_id[4096] = {};
	const hcsr_runtime_status_t id_status = hcsr_runtime_publication_copy_element_id(
			state->active_publication, state->active_generation,
			target.owner_node_epoch, target.owner_node_id, element_id, sizeof(element_id));
	if (id_status != HCSR_RUNTIME_OK
			|| element_id[0] == '\0') {
		return false;
	}
	r_hit = HTMLElementHit();
